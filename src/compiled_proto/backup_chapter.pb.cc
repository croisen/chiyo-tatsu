...
class BackupChapter::_Internal {
 public:
  using HasBits = decltype(std::declval<BackupChapter>()._impl_._has_bits_);
  static constexpr ::int32_t kHasBitsOffset =
    8 * PROTOBUF_FIELD_OFFSET(BackupChapter, _impl_._has_bits_);
  static void set_has_scanlator(HasBits* has_bits) {
    (*has_bits)[0] |= 1u;
  }
  static void set_has_lastpageread(HasBits* has_bits) {
    (*has_bits)[0] |= 2u;
  }
};
