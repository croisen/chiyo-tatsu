...
default:
	...
	@echo "exe_release PREFIX='i686-w32-mingw32-'"
	@echo "Project Root Dir: ${PROJECT_ROOT}"

exe_release: ${WIN32} ${PROTOCPP} ${UTILCO} ${UTILCPPO} ${PROTO_O}
	@echo "Linking ${EXE}"
	@${PREFIX}${CPP} ${CFLAGS}\
		-isystem ${PROJECT_ROOT}libs_win/32/include\
		${RELEASE_FLAGS}\
		--static -m32\
		-o ${EXE}\
		${UTILCO}\
		${UTILCPPO}\
		${PROTO_O}\
		-L${PROJECT_ROOT}libs_win/32/lib/\
...
${PROJECT_ROOT}src/compiled_proto/%.pb.cc: ${PROJECT_ROOT}proto_files/%.proto\
	${WIN32}
	@if [ -e ${PROJECT_ROOT}libs_win/32/bin/protoc.exe ]; then\
...
${PROJECT_ROOT}libs_win/32/lib/libprotobuf.dll.a: ${PROJECT_ROOT}libs_win/32/lib/libabsl_civil_time.dll.a
	@if [ ! -d ${PROJECT_ROOT}libs_win/32/include ]; then\
		mkdir -p ${PROJECT_ROOT}libs_win/32/include;\
		mkdir -p ${PROJECT_ROOT}libs_win/32/lib;\
	fi
...
${PROJECT_ROOT}libs_win/32/lib/libabsl_civil_time.dll.a:
	@if [ ! -d ${PROJECT_ROOT}libs_win/32/include ]; then\
		mkdir -p ${PROJECT_ROOT}libs_win/32/include;\
		mkdir -p ${PROJECT_ROOT}libs_win/32/lib;\
	fi
	@cmake -B ${PROJECT_ROOT}extern/abseil-cpp/build/win32\
		-DCMAKE_INSTALL_PREFIX=${PROJECT_ROOT}libs_win/32\
		-DCMAKE_BUILD_TYPE=Release\
		-DCMAKE_CXX_FLAGS="-m32"\
		-DCMAKE_CXX_STANDARD=20\
	...
	@${MAKE} -C ${PROJECT_ROOT}extern/abseil-cpp/build/win32 -j4 install
	@echo "Built target $@"
